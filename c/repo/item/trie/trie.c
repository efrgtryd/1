...
bool check(char *word)
{
    ...
    int i = strlen(word);
    int p = 0;
    int c = tolower(word[p]) - 'a';
    while(p < i && n->children[c] != NULL)
    {
        c = tolower(word[p]) - 'a';
        n = n->children[c];
        p++;
...
bool check(char *word)
{
    if (n.is_word == true && *word == '\0')
    {
        return true;
    }
    char c = tolower(*word) - 'a';
    n = n.children[c];
    if(n == NULL || (*word == NUL && (*n).is_word == false))
    {
        return false;
    }
    return check(word + 1);
...
bool check(char *word) {
    ...
    while (p < i) {
        ...
        n = n->children[c];
        if (n == NULL) {
            return false; // 如果当前节点不存在，返回 false
        }
        p++;
    }
    return n->is_word; // 如果已经到达单词末尾，检查当前节点是否标记为单词结束
}(gpt的)

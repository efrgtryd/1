...
命令行参数和命令行选项(短- 长--)  optstring：一个字符串，包含程序接受的所有短选项的字符。如果某个字符后面跟着一个冒号（:），则表示该选项需要一个选项参数
...
getopt 会继续解析剩下的参数，直到所有的命令行参数都被处理完毕。如果遇到非选项参数，getopt 会返回 -1，此时 optind 变量会指向第一个非选项参数（默认情况下在命令行中出现在选项参数的后面）。（位置参数，非选项参数，其在命令行中出现的顺序很重要（它们根据在命令行中的位置被解析和处理，顺序很重要））
...
（optind：指示下一次调用 getopt 时应该从 argv 数组的哪个位置开始检查(默认值为1 因为argv[0]为程序名)。当 getopt 处理完所有选项后，optind 会指向第一个非选项参数。）
